...
import static com.rbricks.appsharing.concept.notesapp.db.NewDatabaseManager.softDeleteNotes;
...
public class NewNotesListingPresenter implements BasePresenterInterface {
    ...
    public void fetchNotesList() {
        ...
        lifeCycle.add(RxApiUtil.build(getAllNotes()).subscribe((list) -> {
    ...
    public void fetchSearchedNotesList(String searchString) {
        mvpNotesInterface.showProgress();
        lifeCycle.add(RxApiUtil.build(getAllSearchedNotes(searchString)).subscribe((list) -> {
            mvpNotesInterface.hideProgress();
            mvpNotesInterface.showAllNotes(list);
        }));
    }
    ...
    private void doItemRemoveLogic(Pair<NotesItem, Integer> pair) {
        lifeCycle.add(RxApiUtil.build(softDeleteNotes(pair.first.getId())).subscribe(isSuccess -> {
            if (isSuccess) {
